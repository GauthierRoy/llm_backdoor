...
class AgentLMInfer(ModelInfer):
    ...
    def chat(self,instruct,history):
        ...
        #print("Max token ID:", torch.max(input_ids), "/", self.tokenizer.vocab_size)
        ...
        if torch.max(input_ids) >= self.tokenizer.vocab_size:
            raise ValueError("Token IDs exceed vocab size")
